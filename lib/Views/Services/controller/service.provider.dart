...
class ServiceProvider extends ChangeNotifier {
  ...
  List<ServiceModel> offlineData = [];

  save({
    required ServiceModel data,
  ...
  delete({required ServiceModel data}) {
  ...
  getOffline({bool? isRefresh = false}) async {
    ...
    // print(data.length);
    offlineData = List<ServiceModel>.from(
  ...
  get({bool? isRefresh = false}) async {
    ...
    List<ServiceModel> dataList = List<ServiceModel>.from(
      data.map((item) {
        ServiceModel singleItem = ServiceModel.fromJson(item);
        return singleItem;
      }).toList(),
    );
